...
export default function LoginPage() {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    if (!res.ok) {
      setRegError(data.error || 'เกิดข้อผิดพลาด');
      return;
    }
      ...
      <div className="container" style={{ maxWidth: 480, margin: '0 auto' }}>
        <div style={{ background: 'white', borderRadius: 16, boxShadow: '0 4px 24px rgba(0,0,0,0.08)', overflow: 'hidden' }}>
          ...
          <div style={{ background: 'linear-gradient(135deg, #0d9488, #14b8a6)', padding: '32px', textAlign: 'center' }}>
            ...
            <p style={{ color: '#ccfbf1', margin: 0, fontSize: 14 }}>ระบบจัดการนัดหมายสัตว์เลี้ยง</p>
          </div>
          ...
          <div style={{ padding: '32px' }}>
            ...
            {tab === 'login' && (
              <form onSubmit={handleLogin}>
                ...
                <div style={{ marginBottom: 16 }}>
                  <label style={{ display: 'block', marginBottom: 6, fontSize: 14, fontWeight: 600, color: '#374151' }}>อีเมล</label>
                  <input
                ...
                <div style={{ marginBottom: 24 }}>
                  <label style={{ display: 'block', marginBottom: 6, fontSize: 14, fontWeight: 600, color: '#374151' }}>รหัสผ่าน</label>
                  <input
            ...
            {tab === 'register' && (
              <form onSubmit={handleRegister}>
                ...
                <div style={{ marginBottom: 16 }}>
                  <label style={{ display: 'block', marginBottom: 6, fontSize: 14, fontWeight: 600, color: '#374151' }}>ชื่อ-นามสกุล</label>
                  <input type="text" required value={regName} onChange={(e) => setRegName(e.target.value)}
                    placeholder="สมชาย ใจดี" style={inputStyle} />
                </div>
                <div style={{ marginBottom: 16 }}>
                  <label style={{ display: 'block', marginBottom: 6, fontSize: 14, fontWeight: 600, color: '#374151' }}>อีเมล</label>
                ...
                <div style={{ marginBottom: 16 }}>
                  <label style={{ display: 'block', marginBottom: 6, fontSize: 14, fontWeight: 600, color: '#374151' }}>เบอร์โทรศัพท์</label>
                ...
                <div style={{ marginBottom: 16 }}>
                  ...
                  <input type="password" required value={regPassword} onChange={(e) => setRegPassword(e.target.value)}
                    placeholder="อย่างน้อย 8 ตัวอักษร" style={inputStyle} />
                </div>
                <div style={{ marginBottom: 24 }}>
                  <label style={{ display: 'block', marginBottom: 6, fontSize: 14, fontWeight: 600, color: '#374151' }}>ยืนยันรหัสผ่าน</label>
